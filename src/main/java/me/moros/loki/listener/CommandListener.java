...
public record CommandListener(Consumer<Record> consumer) implements PlayerProcessor, Listener {
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  private void onPlayerCommand(PlayerCommandPreprocessEvent event) {
    consumer.accept(process(Record.create(event.getMessage()), event.getPlayer()));
  }
}
